module aoxc::sentinel_dao {
    ...
    entry fun finalize_proposal(
        ...
        if (p.action_type == ACTION_PAUSE_PROTOCOL) {
            circuit_breaker::pause_from_module(breaker, copy p.payload_hash);
        };
        if (p.action_type == ACTION_RESUME_PROTOCOL) {
            circuit_breaker::resume_from_module(breaker, copy p.payload_hash);
        };

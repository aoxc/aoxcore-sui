module aoxc::sentinel_dao {
    ...
    entry fun queue_proposal(
        ...
        let action_type = bridge_payload::action_type(&action);
        validate_action_type(action_type);
        validate_policy_limits(dao.timelock_ms, requested_fund_delta_bps);

        let id = dao.next_id;
        dao.next_id = dao.next_id + 1;
        let base_eta = clock::timestamp_ms(clock) + dao.timelock_ms;
        let tier2_eta = clock::timestamp_ms(clock) + CRITICAL_UPDATE_TIMELOCK_MS;
        let mut eta_ms = base_eta;
        if (action_type == bridge_payload::kind_fund_update()) {
            if (tier2_eta > eta_ms) { eta_ms = tier2_eta; };
        };

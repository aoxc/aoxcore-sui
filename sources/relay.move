module aoxc::relay {
    ...
    public fun assert_signer_bounds(signers: u16, active_attestors: u64) {
        assert!((signers as u64) <= active_attestors, errors::E_INVALID_ARGUMENT);
    }
    ...
    fun active_attestor_count(quorum: &AttestorQuorum): u64 {
        let mut i = 0;
        let len = vector::length(&quorum.attestor_pubkeys);
        let mut active = 0;
        while (i < len) {
            if (is_attestor_active(quorum, vector::borrow(&quorum.attestor_pubkeys, i))) {
                active = active + 1;
            };
            i = i + 1;
        };
    ...
    entry fun anchor_report(
        ...
        assert_signer_bounds(signer_count, active_attestor_count(quorum));

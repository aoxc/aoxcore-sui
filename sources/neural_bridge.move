module aoxc::neural_bridge {
    ...
    fun command_digest(command: &BridgeCommand): vector<u8> {
        ...
        vector::append(&mut body, b"AOXC_SUI_NEURAL_BRIDGE_PHASE1");
    ...
    /// Verifies command and applies atomic pause/resume directives to circuit breaker.
    public fun verify_and_apply(
        ...
        if (command.command_type == COMMAND_SYSTEM_HALT) {
            circuit_breaker::pause_from_module(breaker, copy digest);
        };
        if (command.command_type == COMMAND_SYSTEM_RESUME) {
            circuit_breaker::resume_from_module(breaker, copy digest);
        };

...
module aoxc::full_flow_tests {
    ...
    fun web4_pipeline_smoke() {
        let intent_raw = bridge_payload::encode_intent_payload(
            ...
            bridge_payload::target_breaker(),
            888,
            string::utf8(b"normal -> anomaly -> safety"),
            9_500,
            2,
            b"flow-proof",
        );
        let _intent = bridge_payload::decode_intent_payload(intent_raw, bridge_payload::kind_system_halt());
        auto_rebalancer::validate_thresholds(100, 400);
        walrus_connector::validate_credential_inputs(&b"issuer", &b"subject", &b"claim", &b"cert");
        let should_slash = staking::should_trigger_relayer_slash(10, 100, false);
        assert!(should_slash, 2);
    }
